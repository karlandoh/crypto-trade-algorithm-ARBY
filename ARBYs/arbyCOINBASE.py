...
mid = ['btcalpha','bitz','tidex','coinegg','okex','oceanex','bitmart','hitbtc'] #removed latoken, southxchange, bitforex
...
class coinbase(balancefetcher):
					...
					if balance >= minimum_balance:
						...
						for execution in cached_balances[:]:
							...
							if execution['currency'].split('/')[0] == currency:
								...
								if currency == 'BTC':
									deposit_table = arby_api.fetch({'method': 'fetch_deposits', 'args':(),'exchange': self.coinbasepro})
									deposit_table = [x for x in deposit_table if x['currency'] == 'BTC' and (x['timestamp']-execution['timestamp']).total_seconds()>0]

									if len(deposit_table) > 0:
										for slot in deposit_table:
											if percent_difference(slot['amount'],execution['total_amount']) <= 5:
												delete_mode = True
												break
									...
									if self.cycle == True:
										...
										if execution['status'] == "TRANSIT":
											execution['status'] = "ORDER"

										if execution['status'] == 'ORDER' and len(retry(5,{'method': 'fetchOpenOrders', 'args':(execution['currency'],), 'exchange': self.coinbasepro})) == 0:
											delete_mode = True
										else:
											print(f"[CYCLE] Waiting for {execution['currency']} to complete!")
										
									if self.cycle == False:
										delete_mode = True

							if delete_mode == True:
								print(f"\n\nDeleting {execution['currency']} !\n\n")
			...
			if cont_signal == 'a':
				...
				while True:
					...
					if len(cached_balances) == 0:
						...
						print('[CYCLE] Waiting for clearance of orders!')
						time.sleep(monitor_coinbase_timer*30)
	...
	def execute(self,best_sequence):
		...
		for execution in best_sequence['sequences']:
			...
			execution['timestamp'] = datetime.datetime.now()
			execution['status'] = "TRANSIT"
