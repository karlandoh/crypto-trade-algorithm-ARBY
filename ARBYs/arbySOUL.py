...
class arbySOUL(trade):
	...
	def determine_balances_after(self,buyorsell,exchange,currency,**kwargs):
		...
		if exchange.has['fetchOpenOrders'] == True or exchange.has['fetchOpenOrders'] == 'emulated':
			try:
				...
				#openorders = retry(f"object[0].fetchOpenOrders('{currency}')",5,exchange)

				openorders = retry(5,{'exchange': exchange, 'method': 'fetchOpenOrders', 'args': (currency,)})
		...
		if exchange.has['fetchOpenOrders'] == False: #BUGGY #BAD! YOU MUST SIMPLY CHECK YOURSELF OR CHECK FOR DONE.
			...
			while True:
				try:
					free_balance = real_input(self.random_id,f"BuyorSell:{buyorsell.upper()}\nCurrency:{currency}\n[TRADE] Please type how much {free_split} is already {action_past} on {exchange.name.upper()}. Can also type 'complete'. ",**self.settings)
	...
	def present_options(self,step,mode,input_balances,buyorsell,exchange,currency,**kwargs): #FIX INPUT BALANCE ARGUMENT?
		...
		end = f"\nPlease choose a number. \nStep = {step} \nCurrency = {self.response['currency']}\nProjected Increase = {self.response['realdifferenceSELL']}\n"
		...
		if step == 1 or step == 5: #DONE
			if mode == 'before':
				...
				if -0.0001 <= (input_balances['initial_balance']-self.response['balance']) <= 0:
					choose = '2'
				...
				while True:
					try:
						choose
						#real_print(self.random_id,input_balances['initial_balance'],**self.settings)
					except:
			...
			if mode == 'after':
				...
				if -0.0001 <= (simulate_sell['balance']-self.response['balance']) <= 0:
					choose = '2'	
				...
				while True:
					try:
						choose
						real_print(self.random_id,str(simulate_sell['balance']-self.response['balance']),**self.settings)
					except:
						choose = real_input(self.random_id,f"[TRADE] {options}",**self.settings)
					if choose == '1':
						return {'option': 'WAIT'}
		...
		if step == 4 or step == 8:
			...
			array = retry(40,{'exchange': eval(f"ccxt.{exchange.id}()"), 'method': 'fetchOrderBook', 'args': (currency,)})['bids']
			...
			if -0.0001 <= (simulate_sell['balance']-self.response['balance']) <= 0:
			#if simulate_sell['balance'] <= 0.0001:
				choose = '2'			

			while True:
				try:
					choose
					#real_print(self.random_id,(simulate_sell['balance']-self.response['balance']),**self.settings)
				except:
					choose = real_input(self.random_id,f"[TRADE] {options}",**self.settings)
	...
	def sendback_action(self,mode,sendexchange,getexchange,currency,**kwargs):
		try:
			...
			try:
				...
				buyarray = retry(10,{'exchange': sendexchange, 'method': 'fetchOrderBook', 'args': (currency,)})
			except TimeoutError:
				return 'CONTINUE'
		...
		try:
			...
			sellarray = retry(20,{'exchange': eval(f"ccxt.{getexchange.id}()"), 'method': 'fetchOrderBook', 'args': (currency,)})
	...
	def simulate_backwards(self,buyorsell,exchange,currency,used_balance,free_balance):
		...
		array = retry(20,{'exchange': eval(f"ccxt.{exchange.id}()"), 'method': 'fetchOrderBook', 'args': (currency,)})
