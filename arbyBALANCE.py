...
class balancefetcher(arbySOUL):
	def __init__(self,internalmode=True):
		...
		self.coinbasepro = inject_exchange_info(eval(f"ccxt.coinbase()"),**self.exchange_info)[0]

	def load_all_trades(self):
		all_trades = {}

		for slot in os.listdir(f"{os.getcwd()}/currenttrades"):
			soldier = int(slot.split('currenttrade')[1].split('.txt')[0])
	...
	def fetch_balance(self,soldier):
		...
		try:
			money = retry(f"""object[0].fetchBalance()['{mode}']['{soldier['currency']}']""",5,exchange)
			#set_trace()
			if soldier['comment'] != '':
	...
	def fetch_balance_online(self,soldier): #DONE
		balance = self.fetch_balance(soldier)
	...
	def start(self,**kwargs):
		...
		coinbase_balance = retry(f"""object[0].fetchBalance()['BTC']['free'] """,5,self.coinbasepro)
		transfer_balance = sum([float(x['comment']) for x in self.soldiers.soldiers if x['status']['status']=='Online-Coinbase'])
	...
	def plot(self):
		df = pd.read_excel(open(f"{os.getcwd()}/SoldierInfo.xls", 'rb'), sheet_name='balances')
