...
class tradeSIM():
	...
	def analyzePurchase(self, buyorsell, orderbook, amount, exchange,currency):
		...
		try:
			...
			try:
				...
				if str(minimumvalue).title() == 'None':
					raise KeyError()
			except:
				minimumvalue = '0.0001'
		...
		print(f'My amount to analyze is {amount} | minimumvalue = {minimumvalue}')
		...
		for i,order in enumerate(orderbook):
			...
			print(f"{i} {ordertotal}")
			
			if (order[0]*order[1])<=eval(minimumvalue):
				#set_trace()
			...
			if lastone == True: #ALL DONE! 
				...
				if (order[0]*purchasequantity)<=eval(minimumvalue):
					#set_trace()
					print(f'The purchase is small! ({order[0]} | {order[1]} | {purchasequantity})')
					smallmode = True				
		...
		if mode == 'cache':
			...
			if mode == 'deposit':
				...
				if exchange.id == 'coinbasepro':
					print(f"\n[{mode.upper()}] NOT ADDING INFORMATION FROM COINBASE. CONSTANTLY CHANGING!")
					return None

			a = {"timestamp": time.time(), "exchange": exchange.id, "currency": currency, "value": value}

			if mode == 'confiscate':
				path = f'{os.getcwd()}/cacheC.txt'

				recieve_exchange = args[3]
				access = args[4]
				buy_name = args[5]
				sell_name = args[6]

				a = {"timestamp": time.time(), "exchange_1": exchange.id, "exchange_2": recieve_exchange.id, "currency": currency, "value": access, "buy_name": buy_name, "sell_name": sell_name}

				print(f"\n[{mode.upper()}] MANUALLY ADDED offline wallet with parameters: Exchange 1 - {exchange.name}, Exchange 2 - {recieve_exchange}, Currency - {currency}, Value - {access}\n")
	...
	def findCache(self,mode):
		...
		if mode == 'confiscate':
			with open(f'{os.getcwd()}/cacheC.txt', "r") as text_file:
				text_file.seek(0)
				lines = text_file.read().split('\n')
				text_file.close()
		...
		try:		
			while True:
				...
				if result['status'] == 'EMPTY':
					...
					if self.settings['modes']['sleepmode'] == True:
			...
			for exchange,info in self.onlineinfo.items():
				if exchange == sendexchange.id:
					try:
						...
						if online_fee == 'NONE' or online_fee == 'huh':
							pass
						else:
	...
	def recommend(self,result_sell,balance):
		...
		if levelbalance == -3:
			if result_sell < 5*math.pow(10,levelbalance-2): #Geniuses are branded as crazy... $0.2
			#if result_sell < 1*math.pow(10,levelbalance-1): #Geniuses are branded as crazy... $1
				return True

		if levelbalance == -2:
			#if result_sell < 2*math.pow(10,levelbalance-3): #Geniuses are branded as crazy... $0.2
			if result_sell < 2*math.pow(10,levelbalance-2): #$2
				return True		
	...
	def online_prepare(self,entry,homeexchange): #IT NEEDS TO STAY HERE. It is an integrated function pulling from lots of different areas.
		...
		#set_trace()
		#PRESCREEN!
		a = self.scan_online(entry['sellexchange'],entry['currency'],'sell','deposit') #REAL
		if a['status'] == 'OFFLINE':
			return a

		b = self.scan_online(entry['buyexchange'], entry['currency'],'buy','withdraw') #REAL
		if b['status'] == 'OFFLINE':
			return b

		#CODE STARTS HERE!
		a = self.prepare(entry['sellexchange'],entry['currency'],'sell','deposit') #REAL
		if a['status'] == 'OFFLINE': #No sell exchange? Bun everything.
			return a

		b = self.prepare(entry['buyexchange'], entry['currency'],'buy','withdraw') #REAL
		if b['status'] == 'OFFLINE':
			return b
		...
		for base in quicktrack[:]:
			...
			try:
				if any(currency.split('/')[0] in x for x in entry['buyexchange'].symbols) and self.prepare(entry['buyexchange'], currency, 'buy','deposit')['status'] == 'SUCCESS':
					...
					print(f'[FASTTRACK] [BUYEXCHANGE] Removing {currency}!')
			...
			if any(currency.split('/')[0] in x for x in homeexchange.symbols) and self.prepare(homeexchange, currency,'buy','withdraw')['status'] == 'SUCCESS':
				pass
			else:
				print(f'[FASTTRACK] [HOMEEXCHANGE] Removing {currency}!')
				quicktrack.remove(base)
				continue
