	...
	def entryWORK(self,input_info):
		...
		for i,shithole in enumerate(self.shitlist):
			...
			if time.time()-shithole['timestamp'] >= 300: #magic_seclimit
				print('\n\n\n\n\n[SHITLIST] Current shithole ENTRY is old! Removing!\n\n\n\n\n')
				del self.shitlist[i]
				return 'CONTINUE'
		...
		if (12 <= datetime.datetime.today().hour < 21) and entry['buyexchange'].id == 'coinegg': #COINEGG HOURS!
			return 'FAIL'
		...
		try:
			...
			#set_trace()
			retry(10,{'exchange': entry['buyexchange'], 'method': 'load_markets', 'args': ()})
		...
		try:
			...
			#set_trace()
			retry(10,{'exchange': entry['sellexchange'], 'method': 'load_markets', 'args': ()})
