...
if __name__ == '__main__':
	...
	if automode == False:
		...
		while True:
			...
			if auto == 'a':
				try:
					money = arby_api.fetch({'method': 'fetchBalance', 'args':(), 'exchange': homebase})['free']['BTC']
					#money = arby_api.fetch("object[0].fetchBalance()['free']['BTC']",5,homebase) #selenium_patch
					break
				except Exception as e:
					try:
	...
	try:
		...
		retry(5,{'exchange': homebase, 'method': 'load_markets', 'args': ()})
	...
	mysettings = {'holyshit': holyshit, 'homebase': homebase,
				  'modes': {'internalmode': internalmode, 'automode': automode, 'capitomode': capitomode, 'sleepmode': False},
