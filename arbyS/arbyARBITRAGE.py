...
def timeCheck(mode):
	...
	if mode == 'confiscate':
		pathway = f'{os.getcwd()}/cacheC.txt'
		week = 8
	...
	for line in lines[:]:
		if currenttime-line['timestamp']>limit:
			...
			if mode == 'deposit' and 'bittrexacct1' in line['value']['address']:
				pass
			else:
		...
		#THIS LINE IS HOW WE TAG FOR DUPLICATES.
		if mode == 'normal':
			try:
				line['depositmode']
		...
		elif mode == 'confiscate':
			line['temp_counter'] = f"{line['currency']}-{line['exchange_1']}-{line['exchange_2']}"
	...
	for temp_counter,number in duplicates.items():
		...
		for line in reversed(lines[:]):
			parse_text = temp_counter.split('-')

			currency = parse_text[0]

			if mode == 'confiscate':
				exchange_1 = parse_text[1]
				exchange_2 = parse_text[2]

				if line['currency'] == currency and ((line['exchange_1'] == exchange_1 and line['exchange_2'] == exchange_2) or (line['exchange_1'] == exchange_2 and line['exchange_2'] == exchange_1)):
					newlines.append(line)
					break

			else:
				exchange = parse_text[1]
				

				if line['currency'] == currency and line['exchange'] == exchange:

					if mode == 'normal':
						
						depositorwithdraw = parse_text[2]

						try:
							line[f"{depositorwithdraw}mode"]
						except KeyError:
							continue
			...
			for i,thread in enumerate(threads):
				if thread['thread'].isAlive() == False:
					if thread['mode'] == 'updator':
						...
						t = threading.Thread(target=self.update_magic,args=args)
						t.start()
						threads[i]['thread'] = t						
