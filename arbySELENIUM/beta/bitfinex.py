...
class exchange():
	...
	def ci_chart(self):
		try:
			...
			for i,slot in enumerate(table):
				...
				info['info'][currency] = {'deposit': True, 'withdraw': False, 'depositinfo': {'address': 'NONE', 'memo': 'NONE'}, 'withdrawinfo': {'minimum': 'NONE', 'fee': 'NONE', 'ethereum_mode': 'NONE'}, 'discard': {'currency_name': slot.text.strip().replace('\n','').split('  ')[-1].replace('-',' ')}}
			...
			for i,slot in enumerate(table):
				...
				try:
					...
					info['info'][currency] = {'deposit': False, 'withdraw': True, 'depositinfo': {'address': 'NONE', 'memo': 'NONE'}, 'withdrawinfo': {'minimum': 'NONE', 'fee': 'NONE', 'ethereum_mode': 'NONE', 'discard': {'currency_name': slot.text.strip().replace('\n','').split('  ')[-1].replace('-',' ')}}}
	...
	def ci_withdraw(self,info={'info': {}}):
		...
		soup = bs4.BeautifulSoup(requests.get('https://www.bitfinex.com/fees').text,'lxml')

		deposit_table = soup.find_all('tbody')[4].find_all('tr')
		...
		for currency,information in info['info'].items():
			...
			for i,slot in enumerate(deposit_table):
				slot_information = slot.find_all('td')
				#getexchange.fees['funding']['deposit'][currency]
				#print(information['currency_name'])
				#print(slot_information[0])

				if information['discard']['currency_name'].lower() == slot_information[0].text.lower().replace('-',' '):
					try:
						deposit_fee = float(slot_information[2].text.strip().split(' ')[0])
					except ValueError:
						if slot_information[2].text.strip().split(' ')[0] == 'FREE':
							deposit_fee = 0

					information['depositinfo']['fee'] = deposit_fee
					done = True

					try:
						information['discard']['real_symbol'] = slot_information[2].text.strip().split(' ')[1]
					except IndexError:
						information['discard']['real_symbol'] = currency
		...
		withdraw_table = soup.find_all('tbody')[5].find_all('tr')
		...
		for currency,information in info['info'].items():
			...
			for i,slot in enumerate(withdraw_table):
				slot_information = slot.find_all('td')
				#getexchange.fees['funding']['deposit'][currency]
				#print(information['discard']['real_symbol'].lower())
				#print(slot_information[1].text.lower())
				withdraw_fee = None
				#import ipdb
				#ipdb.set_trace()
				currency_information = slot_information[1].text.strip()

				try:
					if information['discard']['real_symbol'] == currency_information.split(' ')[1]:
						found = True
						withdraw_fee = float(currency_information.split(' ')[0])
					else:
						continue
				except IndexError:
					if currency_information == 'FREE':
						continue
				except:
					print(currency_information)
					raise

				information['withdrawinfo'] = {'fee': withdraw_fee, 'minimum': 'NONE'}
			...
			if found == False:
				for i,slot in enumerate(withdraw_table):
					...
					#print(information['discard']['currency_name'].lower())
					#print(slot_information[0].text.lower() )
					#time.sleep(0.05)
					#if information['discard']['currency_name'].lower() == 'add':
					#	import ipdb
					#	ipdb.set_trace()

					if slot_information[0].text.lower().strip().replace('-',' ') == information['discard']['currency_name'].lower().strip():

						withdraw_fee = slot_information[1].text.strip().split(' ')[0]
						try:
							withdraw_fee = float(withdraw_fee)
						except ValueError:
							if withdraw_fee == 'FREE':
								withdraw_fee = 0
							else:
								raise
			...
			if found == False:
				if information['discard']['currency_name'].lower() == 'liquidbtc':
					withdraw_fee = information['depositinfo']['fee']
					found = True
			...
			if found == False:
				raise NameError(f'WOW {currency}')

		for currency,information in info['info'].items():
			import ccxt
			bitfinex = ccxt.bitfinex({'apiKey': 'ulaYe4Ae1K5I9Pt07tv6cYkEcOhcyk45gpkjPbcdeT8','secret': 'sMMMY5KJ8MlgwBlGz1WvOMcJPT2ES1u6bKnn6VcOwUZ'})
			bitfinex.enableRateLimit = True

			while True:
				try:
					print(f'Testing {currency}...')
					bitfinex.fetchDepositAddress(currency)
					break
				except Exception as e:
					if "ERR_RATE_LIMIT" in str(e):
						...
						information['withdraw'] = False
						break

		return info

		#for currency,information in info['info'].items():
		#	print(f'wtf {currency}')
		#	real_symbol = information['discard']['real_symbol']
		#	information.pop('discard')
		#
		#	info['info'][real_symbol] = info['info'].pop(currency)
	...
	def update(self):
		info = self.ci_withdraw(self.ci_chart())
		return info
