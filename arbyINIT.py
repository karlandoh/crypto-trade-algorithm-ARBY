...
class arbyINITIALIZE():
	...
	def __init__(self):
		...
		self.global_locks = create_locks() #FIRST LINE!
	...
	def startSOLDIER(self,soldier):
		...
		try:
			money = retry(f"object[0].fetchBalance()['{mode}']['BTC']",5,homebase)
			if money == None:
				raise TimeoutError()
		...
		while True:
			...
			for soldier in soldierinfo:
				...
				if 'Wait' in soldier['status']['status']:
					if (datetime.datetime.now()-soldier['timestamp']).total_seconds() >= 3600:
				...
				if soldier['status']['status'] == 'Online-Replace':
					...
					print(f"[SCREEN] Acquiring online lock!")
					self.locks['withdraw'].acquire()	
					print(f"[SCREEN] Acquiring withdraw lock!")
					try:
