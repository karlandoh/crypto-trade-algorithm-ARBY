...
class execute():
	...
	def cyclePing(self,valueping,pid):
		#setproctitle(f'[{self.session}] CyclePing')
		...
		while True:
			print(f'\nCOUNTDOWN = {countdown}\n')
			if info['Repeat'] == True and valueping.value == 1:
				countdown+=1
			else:
				countdown = 0
	...
	def inject(self,x,indicator,datalist):
		setproctitle(f'[{self.session}] Injector #{x}')
	...
	def socketswitch(self,valueping):
		...
		def find(self):
			...
			try:
				if 'Resource not found' in requests.get(f'https://api.huobi.pro/market/',timeout=tout).text:
	...
	def timer(self,valueping):
		setproctitle(f'[{self.session}] Timer')
	...
	def monitor(self,datalist,valueping):
		setproctitle(f'[{self.session}] Monitor')

		k = threading.Thread(target=self.cyclePing, args=(valueping,os.getpid(),), name = 'CYCLE PING')
		k.daemon = True
	...
	def start(self,limitlist,valueping,datalist): #THIS IS A ONE TIME RUN! SPAWN THE PARALLEL UNIVERSES. VIA POOL
		setproctitle(f'[{self.session}] Start')
		#from serverGRID import gridcreator

		print(limitlist)

		i1 = threading.Thread(target=self.timer, args=(valueping,), name = 'TIMER')
		i1.daemon = True
		i1.start()
		...
		if valueping.value == 0:
			...
			print(f"\nIssue! {valueping.value}\n")

		for wolf in mywolves:
			wolf['process'].terminate()
			wolf['process'].join()
			#print(f"sudo kill -9 {wolf['process'].pid}")
			#os.system(f"sudo kill -9 {wolf['process'].pid}")

	def watchwolf(self,currencydigits,valueping,datalist):
		setproctitle(f'[{self.session}] Watchwolf')
		...
		for digit in currencydigits:
			...
			if any(self.allcurrencies_symbol[digit] == bannedcurrency for bannedcurrency in currencybanlist):
				continue
	...
	def watchdog(self,currency,datalist):#,datalist,queue):
		...
		for j in range(len(self.exchanges)):
			...
			exchange.apiKey = self.exchanges[j].apiKey
			exchange.secret = self.exchanges[j].secret
...
class threadpuppy():
	...
	def run(self):
		...
		if '/BTC' in specialmode:
			print(f'Added from {self.exchange.id.upper()}')
		myqueue.put(parse)
