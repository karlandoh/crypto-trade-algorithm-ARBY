...
class postgresql(): #THIS DATABASE IS THE WHEEL.
	def __init__(self):
		self.login_info = f"dbname='online' user= 'postgres' host='localhost' password='Ghana111' port='5432'"
	...
	def add(self,exchange,info,**kwargs):
		...
		os.system(f"touch {os.getcwd()}/done/txt_files/{exchange}.txt")

		with open(f"{os.getcwd()}/done/txt_files/{exchange}.txt", "w") as text_file:
			text_file.seek(0)
